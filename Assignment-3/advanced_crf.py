            ...
            for utterance in var:
                ...
                if(utterance[2] is not None):
                    for y in range(len(utterance[2])):
                        ...
                        if(utterance[2][y][0] not in stopwords):
                            temp.append('TOKEN_'+utterance[2][y][0])
                            temp.append('POS_'+utterance[2][y][1])
                        #####
                        if(y<len(utterance[2])-1):
                            temp.append('BIGRAM_'+utterance[2][y][0].lower() +' '+utterance[2][y+1][0].lower()) 
                            temp.append('TFIDF_'+utterance[2][y][1] +' '+utterance[2][y+1][1])
                else:
                    temp.append('NONE')
                    temp.append('NONE')   
...
trainer.train('cs544-assign3-advance.crfsuite')

tagger = pycrfsuite.Tagger()
tagger.open('cs544-assign3-advance.crfsuite')
